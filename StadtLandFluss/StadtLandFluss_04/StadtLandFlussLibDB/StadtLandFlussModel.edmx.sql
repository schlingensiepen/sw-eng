...
-- --------------------------------------------------
-- Date Created: 11/30/2019 16:22:31
-- Generated from EDMX file: C:\Work\sw-eng\StadtLandFluss\StadtLandFluss_04\StadtLandFlussLibDB\StadtLandFlussModel.edmx
-- --------------------------------------------------
...
IF OBJECT_ID(N'[dbo].[FK_HauptStadtVon]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[NamedObjects_Stadt] DROP CONSTRAINT [FK_HauptStadtVon];
GO
...
CREATE TABLE [dbo].[NamedObjects_Stadt] (
    [Id] int  NOT NULL,
    [HauptstadtVon_Id] int  NULL,
    [LiegtIn_Id] int  NOT NULL
);
GO
...
-- Creating foreign key on [HauptstadtVon_Id] in table 'NamedObjects_Stadt'
ALTER TABLE [dbo].[NamedObjects_Stadt]
ADD CONSTRAINT [FK_HauptStadtVon]
    FOREIGN KEY ([HauptstadtVon_Id])
    REFERENCES [dbo].[NamedObjects_Land]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
GO

-- Creating non-clustered index for FOREIGN KEY 'FK_HauptStadtVon'
CREATE INDEX [IX_FK_HauptStadtVon]
ON [dbo].[NamedObjects_Stadt]
    ([HauptstadtVon_Id]);
GO

-- Creating foreign key on [LiegtIn_Id] in table 'NamedObjects_Stadt'
ALTER TABLE [dbo].[NamedObjects_Stadt]
ADD CONSTRAINT [FK_LandStadt]
    FOREIGN KEY ([LiegtIn_Id])
    REFERENCES [dbo].[NamedObjects_Land]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
GO
...
-- Creating foreign key on [GeborenIn_Id] in table 'People'
ALTER TABLE [dbo].[People]
ADD CONSTRAINT [FK_StadtPerson]
    FOREIGN KEY ([GeborenIn_Id])
    REFERENCES [dbo].[NamedObjects_Stadt]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
GO
